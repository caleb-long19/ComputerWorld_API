...
func ValidateProductInputs(product *models.Product) error {
	...
	if changed {
		...
		if err != nil {
			// Return an internal api error if the check fails
...
func isValidProductInput(product *models.Product) error {
	...
	validIDPattern := `^[0-9]+$`
	matchedID, _ := regexp.MatchString(validIDPattern, product.UID)
	if !matchedID {
...
func hasProductChanged(product *models.Product) (bool, error) {
	...
	err := db.Init().First(&existingProduct).Error
	if err != nil {
...
func productExists(product *models.Product) (string, error) {
	...
	err := db.Init().Where("product_code = ?", product.ProductCode).First(&productWithCode).Error
	if err == nil {
		// Product code exists
		return "product_code", nil
	}
	if !errors.Is(err, gorm.ErrRecordNotFound) {
		return "", responses.NewHTTPError(http.StatusInternalServerError, "internal api error")
	}

	// Check if the product name already exists
	var productWithName models.Product
	err = db.Init().Where("product_name = ?", product.ProductName).First(&productWithName).Error
	if err == nil {
		// Product name exists
...
func manufacturerIDExists(product *models.Product) (bool, error) {
	...
	err := db.Init().Where("manufacturer_id = ?", product.ManufacturerUID).First(&manufacturer).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			// Manufacturer does not exist
			return false, nil
		}
		return false, responses.NewHTTPError(http.StatusInternalServerError, "internal api error")
	}
