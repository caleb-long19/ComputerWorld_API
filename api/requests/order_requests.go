...
func isValidOrderInput(order *models.Order) error {
	...
	validIDPattern := `^[0-9]+$`
	matchedID, _ := regexp.MatchString(validIDPattern, order.UID)
	if !matchedID {
...
func orderExists(order *models.Order) (bool, error) {
	...
	err := db.Init().Where("order_ref = ?", order.OrderRef).First(&order).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			// Order not found, return false
			return false, nil
		}
		return false, responses.NewHTTPError(http.StatusInternalServerError, "Internal api error: Try again!")
	}
...
func productIDExists(order *models.Order) (bool, error) {
	...
	err := db.Init().Where("product_id = ?", order.ProductUID).First(&product).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return false, nil
		}
